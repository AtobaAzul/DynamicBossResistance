...
env.AddComponentPostInit("combat", function(self)
    ...
    function self:GetAttacked(attacker, damage, weapon, stimuli, ...)
        ...
        if self.inst ~= nil and self.inst:HasTag("epic") and
            attacker:HasTag("player") and not (self.inst.prefab == "toadstool" or (weapon ~= nil and (weapon.prefab == "blowdart_pipe" or weapon.prefab == "ruins_bat" or weapon:HasTag("slingshot") or weapon.prefab == "fence_rotator" or weapon:HasTag("vetcurse_item")))) then
            if NearbyPlayers <= 1 then
                return vanillaGetAttacked(self, attacker, damage, weapon, stimuli, ...)
            else
                if NearbyPlayers >= 10 then NearbyPlayers = 10 end

                if env.GetModConfigData("static_scaling") then
                    --print("static")
                    ...
                    if min_dmg <= 0.25 then min_dmg = 0.25 end

                    if damage < old_damage * min_dmg then
                        damage = old_damage * min_dmg
                    end

                   --print"res", self.inst.damage_res)
                   --print"min dmg", min_dmg)
                   --print"old damage", old_damage)
                   --print"damage", damage)
                end
...
env.AddPrefabPostInitAny(function(inst)
    if inst:HasTag("epic") then
        ...
        inst:DoPeriodicTask(2.5, function(inst)
            ...
            if inst.damage_res > 1 then inst.damage_res = 1 end
        end)
    end
end)
